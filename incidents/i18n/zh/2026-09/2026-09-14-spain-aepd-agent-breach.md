...
**申报内容。** 据 AEPD 文章（**9 月 14 日**由 AEPD 副局长 Francisco Pérez Bes 署名发布）：攻击 agent「先在一批通用文件中搜索漏洞，并完成了一次成功的登录。进入系统后，它开始在应用中自主搜寻漏洞，找到后得以修改个人数据并访问发票」。受害机构、模型名称与初始访问的取得方式均**未披露**；BleepingComputer 指出管理当局**尚未就此展开调查并核实**信息，TechRadar 也强调目前所知甚少、调查仍在进行。
