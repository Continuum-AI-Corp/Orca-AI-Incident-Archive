...
| <a id="cred"></a>`CRED` | Credential abuse | Credential abuse | 56 | Credentials read, carried out or abused by an agent. The difference from `EXFIL` is that what is lost is the **key**, not the data. |
| <a id="weapon"></a>`WEAPON` | Agent used as a weapon | Agent used as a weapon | 53 | A human **deliberately** uses an agent as an attack tool. The difference from `ROGUE` is whether there is a hostile operator. |
...
| <a id="supply"></a>`SUPPLY` | Supply-chain poisoning | Supply-chain poisoning | 37 | The attack happens on the agent's **dependency chain**: packages, models, plugins, extensions, repositories and the agent's own config files. |
