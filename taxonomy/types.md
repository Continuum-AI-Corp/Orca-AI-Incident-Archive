...
| <a id="cred"></a>`CRED` | Credential abuse | Credential abuse | 60 | Credentials read, carried out or abused by an agent. The difference from `EXFIL` is that what is lost is the **key**, not the data. |
| <a id="weapon"></a>`WEAPON` | Agent used as a weapon | Agent used as a weapon | 55 | A human **deliberately** uses an agent as an attack tool. The difference from `ROGUE` is whether there is a hostile operator. |
...
| <a id="infra"></a>`INFRA` | Agent infrastructure exposure | Agent infrastructure exposure | 45 | The agent's **runtime infrastructure** is exposed: inference services, vector stores, orchestration platforms, agent gateways. |
...
| <a id="exfil"></a>`EXFIL` | Data exfiltration | Data exfiltration | 29 | Data actually **leaves** the trust boundary. A demo that gets only as far as "it can read" does not count; there must be an outbound channel. |
