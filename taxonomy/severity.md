...
|---|---|---|---|
| `incident` Incident | 138 | A real-world event (whether or not there was an attacker) | Yes |
| `vulnerability` Vulnerability disclosure | 46 | A vulnerability disclosure. `real_harm` depends on evidence of exploitation in the wild | Yes |
| `research` Research demo | 85 | A controlled demonstration by a research organisation or a vendor. Listed to mark **when an attack surface became public** | No |
...
|---|---|---|
| `true` | 127 | There is a **confirmed victim**: an organisation, data, funds or a system suffered actual damage |
| `false` | 142 | A research demo, or a vulnerability that was fixed with no known exploitation in the wild |
...
| Value | Records | Meaning |
|---|---|---|
| `confirmed` Confirmed | 345 | A primary source confirms AI involvement |
